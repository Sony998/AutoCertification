...
while True:
    ...
    primera = df.iloc[fila_actual + 5, 1:5].astype(float).tolist()
    print(primera)
    segunda = df.iloc[fila_actual + 6, 1:5].astype(float).tolist()
    ...
    errores = df.iloc[fila_actual + 7, 1:5].astype(float).tolist()
...
def agregar_imagenes_pdf4(img_fondo_path, img_superior_path1, img_superior_path2, output_pdf_path, yinferior, ysuperior, error_promedio, desviacion):
    ...
    x_fondo = 0  # Colocar el fondo en 0 para ocupar toda la página
    xsuperior = (carta_ancho - image_inferior.width) // 2 - 20
    ...
    pdfmetrics.registerFont(TTFont('Arial', 'Arial.ttf'))
    c.setFont("Arial", 11)
    c.drawString(350, 674, "{:.2f}".format(float(f"{error_promedio:.2f}")))
...
def agregar_imagenes_pdf3(fondo_path, output_pdf_path, incertidumbre, incertidumbre_expandida, primera, segunda, errores_list):
    ...
    pdfmetrics.registerFont(TTFont('ArialI', 'ArialI.ttf'))
    c.setFont("ArialI", 14)
    c.drawString(312, 376, "10.0")
    c.drawString(313, 318, "10.0")
    c.drawString(313, 302, "10.1")
    c.drawString(313, 285, "10.0")
    c.drawString(313, 268, "10.0")
    c.drawString(313, 252, "10.0")
    c.drawString(313, 236, "10.1")
    c.drawString(313, 218, "10.0")
  ...
  #  c.drawString(330, 378, "{:.2f}".format(float(f"{incertidumbre:.2f}")))
    c.setFont("ArialI", 10)
    for i in range(4):
        c.drawString(260 + i * 40, 88 , "{:.2f}".format(primera[i]))
    for i in range(4):
        c.drawString(260 + i * 40, 63 , "{:.2f}".format(segunda[i]))
    for i in range(4):
        c.drawString(260 + i * 40, 35 , "{:.2f}".format(errores_list[i]))
    c.save()
...
for certficado, error_promedio, desviacion in zip(certificados, errores_list, desviaciones):
    ...
    agregar_imagenes_pdf4(img_fondo_path4, img_superior_path1, img_superior_path2, output_pdf_path, 
                         yinferior=152, ysuperior=388, error_promedio=errores_promedio[errorpos], desviacion=desviacion)
    errorpos += 1
